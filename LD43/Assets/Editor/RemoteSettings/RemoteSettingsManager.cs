...
namespace UnityEngine.Analytics
{
    ...
    {
        private readonly static string k_PluginVersion = "0.1.7";
        ...
        //Link URLs
        private const string k_DocumentationURL = "https://github.com/UnityTech/RemoteConfigEditor";
        ...
        private void AddFooterButtons()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button(m_RefreshButtonContent, GUILayout.Width(EditorGUIUtility.currentViewWidth / 2)))
                {
                    string filePath = System.IO.Path.Combine(Application.persistentDataPath, k_PathUnity);
                    filePath = System.IO.Path.Combine(filePath, m_AppId);
                    filePath = System.IO.Path.Combine(filePath, k_PathAnalytics);
                    filePath = System.IO.Path.Combine(filePath, k_PathConfig);
        ...
        private void SubmitRequest(bool explicitlyRequested = false)
        {
            using (WebClient client = new WebClient())
            {
                ...
                {
                    EditorUtility.DisplayDialog(k_ServerErrorDialogTitle, ex.Message, k_ServerErrorDialogBtnLabel);
                ...
                List<ApplicationConfig> appConfigs = JsonUtility.FromJson<ApplicationConfigs>(result).list;

                foreach (var appConfig in appConfigs)
                {
                    m_EnvironmentNames.Add(appConfig.name);
                ...
                if (string.IsNullOrEmpty(m_RSId))
                {
                    if (appConfigs.Count > 0)
                    {
                        ...
                        EnvironmentSelectionCallback(appConfig.name);
                        m_RSId = appConfig.id;
                    }
                    else
                    {
                ...
                url = string.Format(k_RemoteSettingsPath, m_AppId, m_RSId);
                try
                {
                    result = client.DownloadString(new Uri(url));
                    result = "{ \"list\": " + result + "}";
                    List<RemoteSettingsKeyValueType> remoteSettings = JsonUtility.FromJson<RemoteSettingsData>(result).list;
